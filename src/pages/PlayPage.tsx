...
export function PlayPage({ data, playRun }: any) {
  ...
  const allowedGamesForGroup = useMemo(() => {
    if (!groupId) return Object.keys(gameLabels) as GameType[];
    const mapped = (data.groupGameMaps ?? []).filter((m: any) => m.groupId === groupId).map((m: any) => m.gameType as GameType);
    return mapped.length > 0 ? mapped : (Object.keys(gameLabels) as GameType[]);
  }, [data.groupGameMaps, groupId]);

  useEffect(() => {
    if (!allowedGamesForGroup.includes(gameType)) {
      setGameType(allowedGamesForGroup[0] ?? "slot");
    }
  }, [allowedGamesForGroup, gameType]);
  ...
  const resolveRun = (result: PlayResult) => {
    if (!result) {
      setResultText("No users in this group (or game is disabled for this group).");
      ...
      <div className="grid">
        ...
        <label>
          ...
          <select value={gameType} onChange={(e) => setGameType(e.target.value as GameType)}>
            {allowedGamesForGroup.map((value) => (
