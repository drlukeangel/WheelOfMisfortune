...
export const useAppStore = () => {
  ...
  const api = useMemo(
    () => ({
      ...
      deleteGroup: (id: string) =>
        setData((prev) => ({
          ...
          groupGameMaps: prev.groupGameMaps.filter((m) => m.groupId !== id),
      ...
      setGroupGames: (groupId: string, gameTypes: GameType[]) =>
        setData((prev) => {
          const preserved = prev.groupGameMaps.filter((m) => m.groupId !== groupId);
          const replacements: GroupGameMap[] = gameTypes.map((gameType) => ({ id: uid(), groupId, gameType }));
          const allowedSet = new Set(gameTypes.map((gt) => `${groupId}:${gt}`));
          return {
            ...prev,
            groupGameMaps: [...preserved, ...replacements],
            depletionPools: prev.depletionPools.filter((p) => !p.key.startsWith(`${groupId}:`) || allowedSet.has(p.key))
          };
        }),
      ...
      playRun: (groupId: string, gameType: GameType, mode: PickMode): GameRun | null => {
        ...
        const enabledGames = data.groupGameMaps.filter((m) => m.groupId === groupId).map((m) => m.gameType);
        if (enabledGames.length > 0 && !enabledGames.includes(gameType)) return null;
    ...
    [data.memberships, data.depletionPools, data.groupGameMaps]
  );
